...
@FileSystemInfo(type = "fstypegoeshere", // ([a-z0-9]+ only)
		description = "File system description goes here", factory = srcFileSystem.MyFileSystemFactory.class)
public class srcFileSystem implements GFileSystem {
	 ...
	 */
	public srcFileSystem(FSRLRoot fsFSRL, ByteProvider provider) {
		this.fsFSRL = fsFSRL;
	...
	public void mount(TaskMonitor monitor) {
		monitor.setMessage("Opening " + srcFileSystem.class.getSimpleName() + "...");
	...
	public static class MyFileSystemFactory
			implements GFileSystemFactoryByteProvider<srcFileSystem>,
			GFileSystemProbeByteProvider {

		@Override
		public srcFileSystem create(FSRLRoot targetFSRL,
				ByteProvider byteProvider, FileSystemService fsService, TaskMonitor monitor)
				throws IOException, CancelledException {

			srcFileSystem fs = new srcFileSystem(targetFSRL, byteProvider);
			fs.mount(monitor);
			return fs;
		}
